...
pub fn handle_new_client(trigger: On<Add, ClientOf>, mut commands: Commands) {
    ...
    commands.entity(trigger.entity).insert((
        ...
        MessageReceiver::<ClientChestInventoryOpen>::default(),
        MessageReceiver::<ClientChestInventoryPersist>::default(),
    ));
    // Import message types used above (re-exported from protocols)
    use api::core::network::protocols::{
        ClientBlockBreak, ClientBlockPlace, ClientChatMessage, ClientChestInventoryOpen,
...
pub fn handle_client_disconnected(
    ...
    // Close the transport link immediately so in-memory transports do not keep
    // polling a disconnected channel and emit repeated error logs.
