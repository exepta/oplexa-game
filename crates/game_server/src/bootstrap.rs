...
use crate::world_spawn::ensure_world_spawn_generated;
...
pub fn spawn_server(mut commands: Commands, mut config: ResMut<ServerBootstrapConfig>) {
    #[cfg(feature = "integrated")]
    if let Some(integrated_server_io) = config.integrated_server_io.take() {
        let server_entity = commands
            .spawn((
                Name::new("IntegratedServer"),
                NetcodeServer::new(
                    NetcodeConfig::default()
                        .with_client_timeout_secs(config.netcode_client_timeout_secs),
                ),
                LocalAddr(crate::integrated_server_addr()),
            ))
            .id();

        let client_link_entity = commands
            .spawn((
                Name::new("IntegratedClientLink"),
                LinkOf {
                    server: server_entity,
                },
                LocalAddr(crate::integrated_server_addr()),
                PeerAddr(crate::integrated_client_addr()),
        ...
        info!(
            "Lightyear integrated server started using in-memory transport (server={:?}, link={:?}, local_addr={}, peer_addr={})",
            server_entity,
            client_link_entity,
            crate::integrated_server_addr(),
            crate::integrated_client_addr()
        );
        return;
    }
...
pub struct ServerBootstrapConfig {
    ...
    #[cfg(feature = "integrated")]
    pub integrated_server_io: Option<lightyear::crossbeam::CrossbeamIo>,
}
