...
struct StructureModelPreloadState {
...
impl Plugin for BlockInternalRegistry {
    ...
    fn build(&self, app: &mut App) {
        app.init_resource::<StructureModelPreloadState>()
            .add_systems(OnEnter(AppState::Preload), start_block_registry)
            .add_systems(
                Update,
                finish_preload_after_structure_models_ready.run_if(in_state(AppState::Preload)),
            );
    }
}
...
fn start_block_registry(
    ...
    mut preload_state: ResMut<StructureModelPreloadState>,
) {
    ...
    preload_state.handles = structure_recipe_registry
        .recipes
        .iter()
        .map(|recipe| asset_server.load(recipe.model_asset_path.clone()))
        .collect();
...
fn finish_preload_after_structure_models_ready(
    asset_server: Res<AssetServer>,
    preload_state: Res<StructureModelPreloadState>,
    ...
    let all_ready = preload_state.handles.iter().all(|handle| {
        matches!(
