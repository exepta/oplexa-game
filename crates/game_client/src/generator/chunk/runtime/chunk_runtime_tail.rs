fn collect_finished_collider_builds(
    ...
    let ingame_apply_cap = game_config
        .graphics
        .chunk_collider_apply_per_frame
    ...
    for (key, task) in pending.0.iter_mut() {
        ...
        if let Some((k, build)) = future::block_on(future::poll_once(task)) {
            done_keys.push(*key);
            mesh_update.pending_collider_versions.remove(key);
    ...
    while applied < apply_cap {
        ...
        let desired_version = mesh_update
            .desired_collider_versions
            .get(&(coord, sub))
            .copied()
            .unwrap_or(0);
        if build.version < desired_version {
            continue;
        }
...
fn schedule_remesh_tasks_from_events(
    ...
    mut local_edit_priority: ResMut<LocalEditMeshPriorityQueue>,
    mut mesh_update: ResMut<MeshUpdateState>,
    ...
    let edit_priority_pool = ComputeTaskPool::get();
    ...
    while let Some(key) = local_edit_priority.0.pop_front() {
        priority_keys.insert(key);
    }
    let priority_count = priority_keys.len();
    let sync_priority_budget = if in_game_immediate {
        priority_count.clamp(0, 2)
    } else {
        0
    };
    let can_run_immediate = in_game_immediate
        && ((priority_count > 0 && pending_mesh.0.len() < max_inflight_mesh.saturating_add(6))
            || (priority_count == 0 && total_sub_events <= 8 && pending_mesh.0.len() < 8));
    if can_run_immediate {
        immediate_budget = if priority_count > 0 {
            priority_count.saturating_sub(sync_priority_budget).clamp(0, 4)
        } else {
            total_sub_events.clamp(1, 4)
        };
    }
    let prioritized_inflight_cap = max_inflight_mesh.saturating_add(6);
    let mut sync_priority_done = 0usize;
    ...
    for (coord, subs) in by_chunk {
        let Some(chunk) = chunk_map.chunks.get(&coord) else {
            for sub in subs {
                ...
                *desired_version = desired_version.saturating_add(1);
                mesh_update.immediate_mesh_keys.remove(&key);
        ...
        for sub in subs {
            ...
            *desired_version = desired_version.saturating_add(1);
            ...
            immediate_ready.0.retain(|item| item.key != key);
            let wants_immediate = priority_keys.contains(&key);
            let replacing_pending = pending_mesh.0.contains_key(&key);

            if wants_immediate && sync_priority_done < sync_priority_budget {
                let y0 = sub * SEC_H;
                ...
                pending_mesh.0.remove(&key);
                backlog.0.retain(|queued| *queued != key);
                backlog_set.0.remove(&key);
                mesh_update.pending_mesh_versions.remove(&key);
                mesh_update.immediate_mesh_keys.remove(&key);
                immediate_ready.0.push_front(ReadyMeshItem {
                    key,
                    version: desired_version,
                    builds,
                    immediate: true,
                });
                sync_priority_done += 1;
                continue;
            }

            if in_game_immediate
                && immediate_used < immediate_budget
                && (replacing_pending || pending_mesh.0.len() < prioritized_inflight_cap)
                && (wants_immediate || priority_count == 0)
            {
                // Prioritize player edits on worker threads instead of blocking the main thread.
                let y0 = sub * SEC_H;
                let y1 = (y0 + SEC_H).min(CY);
                let borders = snapshot_borders(&chunk_map, coord, y0, y1);
                let chunk_copy = Arc::clone(&chunk_shared);
                let reg_copy = reg_lite.clone();
                let t = edit_priority_pool.spawn(async move {
                    let builds =
                ...
                mesh_update.immediate_mesh_keys.insert(key);
...
fn unload_far_chunks(
    ...
    let center_c = if let Some(cam) = q_cam.iter().next() {
...
fn cleanup_chunk_runtime_on_exit(
    ...
    cleanup.mesh_update.immediate_mesh_keys.clear();
    cleanup.mesh_update.desired_collider_versions.clear();
    cleanup.mesh_update.pending_collider_versions.clear();
