...
pub(super) fn do_connect(
    ...
    reset_runtime_after_connect(runtime, session_url, entity);
}

#[cfg(feature = "integrated")]
pub(super) fn do_connect_integrated(
    runtime: &mut MultiplayerClientRuntime,
    session_url: String,
    integrated_client_io: lightyear::crossbeam::CrossbeamIo,
    commands: &mut Commands,
) {
    if !runtime.enabled {
        return;
    ...
    let server_addr = oplexa_game_server::integrated_server_addr();
    let client_addr = oplexa_game_server::integrated_client_addr();
    let auth = Authentication::Manual {
        server_addr,
        client_id: rand::random::<u64>().max(1),
        private_key: [0u8; 32],
    ...
    let netcode_client = match NetcodeClient::new(auth, netcode_config) {
        ...
        Err(error) => {
            warn!(
                "Failed to create integrated netcode client over in-memory transport: {:?}",
                error
            );
            return;
        }
    ...
    info!(
        "Connecting to integrated server with in-memory transport (player_name={}, client UUID={})",
...
fn reset_runtime_after_connect(
    runtime: &mut MultiplayerClientRuntime,
    session_url: String,
    entity: Entity,
) {
