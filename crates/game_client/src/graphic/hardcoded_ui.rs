...
use crate::generator::chunk::chunk_runtime::{
...
struct ChestInventorySnapshotCache {
    slots_by_world_pos: HashMap<[i32; 3], Vec<ChestInventorySlotPayload>>,
}

#[derive(Resource, Debug, Default, Clone, Copy)]
struct ChestInventorySnapshotRequestState {
    last_requested_world_pos: Option<[i32; 3]>,
    next_request_at_secs: f32,
}

#[derive(Resource, Debug, Default, Clone)]
struct ChestInventoryAutoPersistState {
    chest_loaded: bool,
    last_chest_world_pos: Option<[i32; 3]>,
    last_chest_slots: Vec<ChestInventorySlotPayload>,
    workbench_left_loaded: bool,
    last_workbench_left_world_pos: Option<[i32; 3]>,
    last_workbench_left_slots: Vec<ChestInventorySlotPayload>,
    workbench_right_loaded: bool,
    last_workbench_right_world_pos: Option<[i32; 3]>,
    last_workbench_right_slots: Vec<ChestInventorySlotPayload>,
}
...
impl Plugin for HardcodedUiPlugin {
    ...
    fn build(&self, app: &mut App) {
        app.init_resource::<LoadingProgress>()
            ...
            .init_resource::<ChestInventorySnapshotCache>()
            .init_resource::<ChestInventorySnapshotRequestState>()
            .init_resource::<ChestInventoryAutoPersistState>()
            ...
            .add_systems(
                Update,
                request_chest_inventory_snapshots_for_hud
                    .run_if(in_state(AppState::InGame(InGameStates::Game))),
            )
            .add_systems(
                Update,
                auto_persist_open_container_inventories
                    .run_if(in_state(AppState::InGame(InGameStates::Game))),
            )
            .add_systems(
                Update,
            ...
            .add_systems(
                Update,
                (
                    cache_chest_inventory_contents_sync.in_set(InGameInventoryUiSet::Input),
                    cache_chest_inventory_persist_requests.in_set(InGameInventoryUiSet::Input),
                    prune_chest_inventory_snapshot_cache.in_set(InGameInventoryUiSet::Input),
                )
                    .run_if(in_state(AppState::InGame(InGameStates::Game))),
            )
            .add_systems(
            ...
            .add_systems(
                ...
                (
                    ...
                    clear_chest_inventory_snapshot_cache,
