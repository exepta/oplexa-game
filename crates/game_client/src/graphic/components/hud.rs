...
fn sync_hud_looked_block_card(
    ...
    chest_snapshot_cache: Res<ChestInventorySnapshotCache>,
...
fn build_chest_hud_preview(
    world_pos: IVec3,
    cache: &ChestInventorySnapshotCache,
    item_registry: &ItemRegistry,
) -> Option<ChestHudPreview> {
    let mut ordered_slots = cache
        .slots_by_world_pos
        .get(&world_pos.to_array())
        .cloned()
        .unwrap_or_default();
...
fn request_chest_inventory_snapshots_for_hud(
    time: Res<Time>,
    selection: Res<SelectionState>,
    block_registry: Res<BlockRegistry>,
    ...
    q_structures: Query<&crate::logic::events::block_event_handler::PlacedStructureMetadata>,
    mut request_state: ResMut<ChestInventorySnapshotRequestState>,
    mut requests: MessageWriter<ChestInventorySnapshotRequest>,
) {
    if chest_menu.open || workbench_menu.open {
        request_state.last_requested_world_pos = None;
        request_state.next_request_at_secs = 0.0;
        return;
    }

    let target_world_pos = if let Some(hit) = selection
        .hit
        .filter(|hit| hit.block_id != 0 && is_chest_block_id(hit.block_id, &block_registry))
    {
        Some(hit.block_pos.to_array())
    } else if let Some(structure_hit) = selection.structure_hit {
        q_structures
            .get(structure_hit.entity)
            .ok()
            .filter(|meta| is_chest_structure_meta(meta))
            .map(|meta| [meta.place_origin.x, meta.place_origin.y, meta.place_origin.z])
    } else {
        None
    };

    let Some(world_pos) = target_world_pos else {
        request_state.last_requested_world_pos = None;
        request_state.next_request_at_secs = 0.0;
        return;
    };

    if request_state.last_requested_world_pos == Some(world_pos)
        && time.elapsed_secs() < request_state.next_request_at_secs
    {
        return;
    }

    requests.write(ChestInventorySnapshotRequest { world_pos });
    request_state.last_requested_world_pos = Some(world_pos);
    request_state.next_request_at_secs = time.elapsed_secs() + 0.35;
}
