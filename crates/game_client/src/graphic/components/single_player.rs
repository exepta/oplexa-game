...
fn load_world_and_start(
    ...
    integrated_server: &mut crate::integrated_server::IntegratedServerSession,
    ...
    let session_url = match integrated_server.start(
        world.path.clone(),
        ...
        Err(error) => {
            warn!(
                "Failed to start integrated server for '{}': {}",
                world.folder_name, error
            );
            return false;
        }
    ...
    multiplayer_connection.server_name = Some(format!("{} (Integrated)", world.folder_name));
    multiplayer_connection.world_name = Some(world.folder_name.clone());
    multiplayer_connection.world_seed = Some(world.seed);
    multiplayer_connection.spawn_translation = Some(world_spawn);
    ...
    commands.remove_resource::<WorldSave>();
    commands.remove_resource::<LoadCenter>();
    commands.write_message(ConnectToServerRequest {
        session_url,
        server_name: format!("{} (Integrated)", world.folder_name),
    });
